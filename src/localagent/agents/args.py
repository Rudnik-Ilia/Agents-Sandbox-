...
def parse_agent_flags(description: str, *, rag: bool = False, persist: bool = False) -> argparse.Namespace:
    ...
    if rag:
        parser.add_argument("--no-index", dest="skip_index", action="store_true", help="start without indexing the corpus")
        parser.add_argument("--drop", action="store_true", help="drop persisted RAG storage before starting (Chroma only)")
